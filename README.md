...
동아리 강의를 위한 HTML 발표자료와 학기 포트폴리오 프로젝트다. 1회차는 `brief.md`를 기획 입력으로, `deck.json`을 화면·한국어 발표 원고의 렌더링 정본으로 사용한다. 현재 1회차는 **검토 중인 비공개 초안**이며, 수업 실시나 공개 게시를 뜻하지 않는다.
...
- 검증·브라우저 QA와 기본 비활성 GitHub Pages workflow.

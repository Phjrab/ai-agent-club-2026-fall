...
try{
 ...
 await withBrowser(async({browser,url})=>{
  ...
  const manifest=await page.evaluate(()=>window.presentation.getManifest());add('slide_count',manifest.slides.length===31?'PASS':'FAIL',`${manifest.slides.length}장`);
  ...
  await page.goto(`${url}/lectures/${slug}/index.html#slide=L01-S10`);await page.waitForFunction(()=>window.__PRESENTATION_READY__===true);add('deep_link',(await page.locator('.slide-title').textContent())==='Instructor’s Take: Three Starting Points'?'PASS':'FAIL','L01-S10 직접 접속');
  await page.keyboard.press('ArrowRight');add('keyboard',(await page.locator('.slide-title').textContent())==='Check the Actual Allowance'?'PASS':'FAIL','ArrowRight');
  ...
  await page.goto(`${url}/ai-agent-club-2026-fall/lectures/${slug}/index.html#slide=L01-S01`);await page.waitForFunction(()=>window.__PRESENTATION_READY__===true);add('base_path',await page.locator('.slide-title').textContent()==='From Chat to Action'?'PASS':'FAIL','/ai-agent-club-2026-fall/');
  await page.evaluate(()=>window.presentation.goTo('L01-S15'));add('video_links',(await page.locator('.block-video-card a').count())===3?'PASS':'FAIL','V01 → V02 → V03');

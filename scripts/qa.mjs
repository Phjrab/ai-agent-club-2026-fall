...
try{
 ...
 const renderedDeck=JSON.parse(fs.readFileSync(path.join(root,'dist','private','lectures',slug,'deck.json'),'utf8'));
 ...
 await withBrowser(async({browser,url})=>{
  ...
  await page.evaluate(()=>window.presentation.goTo('L01-S15'));const videoThumbs=await page.locator('.block-video-card .video-thumb img').evaluateAll(imgs=>imgs.map(img=>({loaded:img.complete&&img.naturalWidth===1280&&img.naturalHeight===720,caption:img.closest('figure')?.querySelector('.asset-caption')?.textContent||''})));const expectedThumbs=renderedDeck.slides.find(x=>x.id==='L01-S15').blocks.find(b=>b.type==='video-list').items.filter(item=>item.assetId).length;add('video_links',(await page.locator('.block-video-card a').count())===3?'PASS':'FAIL','V01 → V02 → V03');add('youtube_thumbnail_identity',videoThumbs.length===expectedThumbs&&videoThumbs.every(x=>x.loaded&&x.caption.includes('YouTube 썸네일')&&x.caption.includes('실제 제품 화면'))?'PASS':'FAIL',JSON.stringify({expected:expectedThumbs,actual:videoThumbs}));

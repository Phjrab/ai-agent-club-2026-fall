...
- 이 학기의 슬라이드·발표 원고·템플릿·새 회차 기본 언어는 한국어다. 회차 설정에 다른 언어가 명시된 경우에만 그 설정을 따른다. 제품명·코드·경로·URL·원본 UI 문구는 정확한 표기를 보존한다.
- 시간은 회차별 `duration_minutes`, `questions_minutes`, `questions_mode` 설정에서 계산한다. 질의응답 시간을 코드에서 10분으로 고정하지 않으며, 유동 운영이면 고정 배정은 0분이다.
